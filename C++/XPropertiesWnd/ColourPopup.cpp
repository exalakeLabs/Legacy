...
#define SENDMESSAGE(pWnd, message, wParam, lParam) if (pWnd) if (::IsWindow(pWnd->m_hWnd)) pWnd->SendMessage(message, wParam, lParam);
...
int CColourPopup::m_nNumColours  = sizeof(m_crColours)/sizeof(ColourTableEntry);
                          ...
                          LPCTSTR szCustomText  /* = NULL */)
{
    ...
    if (!CWnd::CreateEx(WS_EX_TOOLWINDOW, szClassName, _T(""), WS_POPUP, 
...
void CColourPopup::ChangeSelection(int nIndex)
{
	...
	{
		SENDMESSAGE(m_pParent,CPN_SELCHANGE, (WPARAM) m_crInitialColour, 0);
	}
    else if (m_nCurrentSel == DEFAULT_BOX_VALUE)
    {
        m_crColour = CLR_DEFAULT;
		SENDMESSAGE(m_pParent,CPN_SELCHANGE, (WPARAM) CLR_DEFAULT, 0);
    }
    else
    {
        m_crColour = GetColour(m_nCurrentSel);
		SENDMESSAGE(m_pParent,CPN_SELCHANGE, (WPARAM) m_crColour, 0);
    }
}
...
void CColourPopup::EndSelection(int nMessage)
{
	...
	SENDMESSAGE(m_pParent,nMessage, (WPARAM) m_crColour, 0);
...
// Function name	: CColourPopup::FindColor
// Description	    : 
// Return type		: int 
// Argument         : COLORREF crColor
int CColourPopup::FindColor(COLORREF crColor)
{
    for (int i = 0; i < m_nNumColours; i++)
        if (GetColour(i) == crColor)
			return i;
	return 0;
}
